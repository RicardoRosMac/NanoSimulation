#############those times are needed 

library(testit)
library(microbenchmark)
k <- c(100, 1000, 10000)
n <- c(10000, 100000, 1000000)
Ptime <- microbenchmark(exect = {
  originales <- rnorm(k)
  ...
  for (paso in 1:duracion) {
    ...
    assert(sum(cumulos) == n)
    assert(length(cumulos[cumulos == 0]) == 0) # que no haya vacios
    freq <- as.data.frame(table(cumulos)) # actualizar exect
    names(freq) <- c("tam", "num")
    freq$tam <- as.numeric(levels(freq$tam))[freq$tam]
...
roturaCum <- function(f) {
  urna <- f[i,]
  if (urna[1,1] > 1) { # no tiene caso romper si no se puede
    cm <- c(romperse(urna[1,1], urna[1,2]))
  } else if (urna[1,1] <1) {
    cm <- rep(1, urna[1,2])
  }
  return(cm)
...
unionCum <-function(f){
  urna <- f[i,]
  cumulos <- c(unirse(urna$tam, urna$num))
  return(cumulos)
}

juntarCum <- function(juntarse){
  c <- c(juntarse[2*i-1] + juntarse[2*i])
  return(c)
}
...
TimeF <- microbenchmark(exect2 = {
  ...
  for (paso in 1:duracion) {
    assert(sum(cumulos) == n)
    cumulos <- integer()
    # cumulos <- foreach(i = 1:dim(freq)[1], .combine="c", .packages="testit") %dopar% roturaCum(freq)
    for (i in 1:dim(freq)[1]) { # fase de rotura
      urna <- freq[i,]
      if (urna$tam > 1) { # no tiene caso romper si no se puede
        cumulos <- c(cumulos, romperse(urna$tam, urna$num))
    ...
    assert(sum(freq$num * freq$tam) == n)
    cumulos <- integer()
    cumulos <- foreach(i = 1:dim(freq)[1], .combine="c", .packages="testit") %dopar% unionCum(freq)
    assert(sum(abs(cumulos)) == n)
    assert(length(cumulos[cumulos == 0]) == 0) # que no haya vacios
    juntarse <- -cumulos[cumulos < 0]
    cumulos <- cumulos[cumulos > 0]
    ...
    png(paste("p8_par", tl, ".png", sep=""), width=300, height=300)
    tope <- 50 * ceiling(max(cumulos) / 50)
    hist(cumulos, breaks=seq(0, tope, 50), 
         main=paste("Paso", paso, "con ambos fen\u{F3}menos"), freq=FALSE,
         ylim=c(0, 0.05), xlab="Tama\u{F1}o", ylab="Frecuencia relativa")
    graphics.off()
  }
}, times = 10, unit = "s")
stopCluster(clust)
...
ggplot(tiempos, aes(x= tiempos$expr, y = tiempos$time, fill = tiempos$expr)) + geom_violin(trim = FALSE) + theme_grey(base_size = 24) +
  guides(fill = FALSE) + labs(x = "Ejecución", y = "Tiempo (segundos)") + scale_x_discrete(labels = c('Paralelo', 'Secuencial')) +
  scale_fill_manual(values=c("lemon chiffon", "mint cream"))
